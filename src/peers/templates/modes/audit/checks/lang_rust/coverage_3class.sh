...
trap 'rm -f "$tmp"' EXIT
find . \
  \( -path './.git' -o -path './.peers' -o -path './target' \) -prune \
  -o -name '*.rs' -type f -print > "$tmp"

test -s "$tmp" || {
  echo "coverage_3class_rust FAIL: no Rust source/test files found"
  exit 1
}
...
if [ -n "$missing" ]; then
  echo "coverage_3class_rust FAIL: missing $missing test class(es)"
  exit 1
fi
echo "coverage_3class_rust: clean"
